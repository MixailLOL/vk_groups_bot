...
import pathlib 
import os
#https://vk.com/botrandoma?w=wall-181476474_765
#https://oauth.vk.com/authorize?client_id=6957898&scope=photos,wall,groups,photos&redirect_uri=http://api.vk.com/blank.html&display=page&response_type=token
...
token = 'vk1.a.hjBdfwgFlyPqaCIyGSiNB5UFZctvkOf6daXb9L837u2ZGYIr3AkxHOqCRmrfNPuaGI-wK5ZKsIQt_Xqjry6CzQV0PYoLlpZ2KKwp172RmvVSdBIpIdoPO6nTkkuEAdowpCkRJ8HF313x1sACW9855FsU3dOuz_8h1OlaQRvCMf02skiJKwfm354IwjgsETRJ'
group_id_cats = 192603988
group_id_rand = 181476474
flickr_url_cat = 'https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=03593667c94923eef10be7a5eca89b13&tags=Cat,kitten,cats&max_upload_date='+str(years)+'-'+str(month)+'-'+str(day)+'&safe_search=1&per_page=300&tag_mode=any&page=1&format=json&nojsoncallback=1'
flickr_url_interesting = 'https://api.flickr.com/services/rest/?method=flickr.interestingness.getList&api_key=03593667c94923eef10be7a5eca89b13&per_page=300&page=1&date='+str(years)+'-'+str(month)+'-'+str(day)+'&safe_search=1&format=json&nojsoncallback=1'
...
def getFlickrPic(url, size):
		...
		getSize = requests.get('https://api.flickr.com/services/rest/?method=flickr.photos.getSizes&api_key=03593667c94923eef10be7a5eca89b13&photo_id='+str(pic_list['photos']['photo'][pic_number]['id'])+'&format=json&nojsoncallback=1').json()
		try:
			url_size = getSize['sizes']['size'][size]['source']
			return(str(getSize['sizes']['size'][size]['source']))
		except:
			return(getFlickrPic(url, size))
...
def main():
	while 1:
		while(1):
				...
				day = str(random.randint(0,2)) + str(random.randint(1,9))
				size = 10
				cat_pic_url = getFlickrPic(flickr_url_cat, size)
				print("pic_url cat = ",cat_pic_url)
				caption =' '.join(get_relevant_cat_tegs(cat_pic_url))
		...
		out.write(img)
		out.close()
		upload_url = getWallUploadServer(group_id_cats)
		...
		wall_post_response = requests.get('https://api.vk.com/method/wall.post?', params ={'attachments':save_result,
																				...
																				'message': caption,
																				'v':'5.101'}).json()
		print(wall_post_response, time.strftime('%H')+':'+time.strftime('%M')+' CATS', years,'-',month,'-',day)
		post_type = random.randint(1,7)
		print("post_type = ", post_type)
		if(post_type == 1):
			upload_url = getWallUploadServer(group_id_rand)
			...
			pic_url = ''
			pic_url = getFlickrPic(flickr_url_interesting, size)
			print("pic_url interest = ",pic_url)
			caption =' '.join(get_relevant_tegs(pic_url))
			...
			wall_post_response = requests.get('https://api.vk.com/method/wall.post?', params ={'attachments':save_result,
																					...
																					'v':'5.101'}).json()
			print(wall_post_response, time.strftime('%H')+':'+time.strftime('%M'), years,'-',month,'-',day)
		timeStop = random.randint(5400, 7200)
		print(timeStop)
		sleep(timeStop)
